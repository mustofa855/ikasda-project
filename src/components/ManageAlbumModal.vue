<template>
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg z-10 w-11/12 md:w-3/4 lg:w-1/2 overflow-y-auto max-h-screen">
            ...
            <!-- Cover Album -->
            <div class="mb-4">
                <img :src="coverPreview ? coverPreview : (editAlbum.cover_image ? editAlbum.cover_image : 'https://via.placeholder.com/600x400')"
                    alt="Album Cover" class="w-full h-64 object-cover rounded" />
            </div>
            ...
            <form @submit.prevent="updateAlbum" class="mb-6">
                <div class="mb-4">
                    ...
                    <input type="text" v-model="editAlbum.title" class="w-full border border-gray-300 p-2 rounded"
                        required />
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 mb-2">Deskripsi Album</label>
                    <textarea v-model="editAlbum.description" rows="3"
                ...
                <div class="flex justify-end">
                    <button type="submit" :disabled="!newCoverFile" :class="[
            ...
            <div class="flex justify-end">
                <button @click="uploadImages" :disabled="!selectedFiles.length" :class="[
...
export default {
    ...
    watch: {
        album: {
            ...
            handler(newVal) {
                if (newVal) {
                    // Set data album yang baru
                    this.editAlbum.title = newVal.title;
                    this.editAlbum.description = newVal.description;
                    this.editAlbum.cover_image = newVal.cover_image;
                    this.coverPreview = ""; // Reset preview cover
                    this.fetchImages(newVal.id);  // Panggil fetchImages berdasarkan album.id yang benar
                }
            }
        }
    },
    methods: {
        ...
        fetchImages(albumId) {
            ...
            axios
                .get(`http://127.0.0.1:8000/api/galleryimages/?album=${albumId}`, {
