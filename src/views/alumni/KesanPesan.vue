  ...
  <div class="mx-4 sm:mx-8 md:mx-16 my-8 p-4 sm:p-6 bg-white shadow-lg rounded-lg">
    ...
    <form @submit.prevent="submitMessage">
      <div class="mb-4">
        ...
        <textarea v-model="message" id="message" rows="4"
          ...
          placeholder="Tuliskan kesan atau pesan Anda di sini"></textarea>
      </div>
      <!-- Tombol Submit -->
      <div class="mt-4">
        <button type="submit"
    ...
    <div class="mt-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        <div v-for="msg in paginatedMessages" :key="msg.id" class="p-4 bg-gray-100 rounded-lg flex flex-col space-y-2">
          <div class="flex items-center space-x-1">
            <!-- Foto profil di samping nama -->
            <img :src="msg.profile_photo ? msg.profile_photo : 'https://via.placeholder.com/40'" alt="Foto Profil"
              class="w-10 h-10 rounded-full object-cover" />
            <p class="mb-0 pl-2"><strong>{{ msg.full_name }}</strong></p>
            <Icon 
              v-if="msg.verified" 
          ...
          <div class="flex items-center space-x-2 mt-auto">
            <button @click="toggleLike(msg)" class="focus:outline-none">
              <Icon v-if="msg.is_liked" icon="mdi:thumb-up" class="text-blue-600" width="20" height="20" />
              <Icon v-else icon="mdi:thumb-up-outline" class="text-gray-600 hover:text-blue-600" width="20"
                height="20" />
            </button>
